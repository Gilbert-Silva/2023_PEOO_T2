class Energia:
  ...
  def valor(self):
    if self.__consumo <= 30: return 10
    if self.__consumo <= 300: return 10 + (self.__consumo - 30) * 1
    return 10 + 270 + (self.__consumo - 300) * 1.5
  def __str__(self):
    return f"Conta de energia {self.__mes}/{self.__ano} - consumo: {self.__consumo}"
